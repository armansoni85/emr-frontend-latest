...
import { getAIChats, createAIChat } from "@src/services/supportAIService";
...
const ChatAIPage = () => {
    ...
    const { data: aiChats, isLoading, isError } = useQuery({
        queryKey: ["aiChats"],
        queryFn: () => getAIChats(),
    ...
    const { mutate: sendMessage, isPending: isSending } = useMutation({
        ...
        onSuccess: (data) => {
            ...
            // Refetch chats to update the chat history
    ...
    const handleSendMessage = () => {
        if (question.trim()) {
            ...
            sendMessage({ question: question.trim() });
        }
    };
                ...
                <div className="bg-white rounded-[20px] shadow-lg mb-4 flex flex-col">
                    ...
                    <div className=" h-full">
                        ...
                        <div className="relative">
                            {isLoading ? (
                                <div className="p-4 text-center text-muted">Loading chat history...</div>
                            ) : isError ? (
                                    ...
                                    >
                                        <div className="flex gap-2 w-full">
                                            ...
                                            <div className="text-medium text-start">
                                                <p className="leading-none">{chat.question || "Chat conversation"}</p>
                                                <span className="text-muted text-sm">{chat.created_at ? new Date(chat.created_at).toLocaleDateString() : "Unknown date"}</span>
                                            </div>
                                        </div>
                                    </div>
                                ))
                            ) : (
                                <div className="p-4 text-center text-muted">No chat history available</div>
                            )}
                        </div>
                    </div>
                    <div className="p-3">
